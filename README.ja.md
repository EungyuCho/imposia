...
ソースをサニタイズし、許可されたアセットを解決し、1つの標準 iframe 内で
ページ分割を行い、その同じ文書をプレビューからネイティブ印刷まで維持します。
...
| :--- | :--- | :--- |
| プレビューと印刷が一致しない | 各画面でレイアウトを再実行する | 1つの標準 iframe をページ分割、表示、ネイティブ印刷まで維持する |
...
| **ページ分割** | ページ形状、対応する `@page` ルール、フラグメンテーション、参照、パブリッシング内容を1つの iframe 内で解決します。 |
| **表示** | Viewer と React 画面はページを複製したりレイアウトを再実行したりせず、標準 iframe を保持します。 |
| **発行** | ネイティブ印刷はその iframe を対象とし、EPUB は最新のコミット済みセマンティックソースを制限付きアーカイブへ投影します。 |

ソース更新に失敗するとアトミックにロールバックします。以前のコミット済み
世代は、置き換えが成功するかコントローラーが破棄されるまで利用できます。
...
| [`@imposia/core`](./packages/core) | 標準ページ文書ランタイム | React なしでライフサイクル、ページ分割、resolver、extension、印刷、EPUB を直接制御する場合 |
...
| 状態 | 含まれる動作 |
| :--- | :--- |
| **Stable** | ブラウザ ESM API、標準 iframe のライフサイクル、resolver 分離、ページ形状、対応 `@page` セレクターとマージンボックス、改ページ、ネイティブ印刷、リフロー型 EPUB 書き出し |
...
ビルド、ブラウザ E2E スイート、依存関係ライセンス監査を実行します。完全な
ゲートと保存済みアーティファクト一覧は
