...
<p align="center">
  ...
  <br/>
  <sub>ブラウザー内でHTML/CSRをアトミックにページ化し、Reactプレビュー、ネイティブ印刷、セマンティックEPUB出力を提供します。</sub>
</p>
...
**宣言済みフローの内容をページ境界で欠落・重複させず、現在のHTMLとCSSを
検査可能なブラウザーページへ変換します。**
...
CSR更新が連続しても、完全な次世代が準備できるまで以前のコミットを表示します。
最新のコミット済みセマンティックソースは、リフロー型 EPUB 3.3 `Blob` としても
書き出せます。
...
| :--- | :--- | :--- |
| 内容がページ境界をまたぐ | 断片が欠落・重複したり、順序が崩れたりする | 宣言済み検証フィクスチャはページを連結するとソース順序と完全に一致し、未対応ケースは成功扱いせずアトミックに保つか警告する |
| ページ化中にCSR状態が変わる | 部分的または古い世代が表示される | ステージング中は以前のコミットを維持し、完全な勝者世代だけで置き換える |
...
| [`@imposia/client`](https://www.npmjs.com/package/@imposia/client) | 統合されたフレームワーク非依存エントリーポイント | 1つのブラウザ専用依存関係から Core と Viewer API を使う場合 |
| [`@imposia/core`](https://www.npmjs.com/package/@imposia/core) | canonical ページ文書ランタイム | React なしでライフサイクル、ページ分割、resolver、extension、印刷、EPUB を直接制御する場合 |
