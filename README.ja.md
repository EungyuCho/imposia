...
<p align="center">
  <strong>ReactのHTMLを印刷し、PDFとして保存。</strong>
  <br/>
  <sub>アプリ内でページを確認し、ブラウザーの印刷画面から紙へ印刷またはPDF保存。EPUB出力にも対応します。</sub>
</p>
...
**使い慣れたHTMLとCSSを、そのままページに。アプリで確認した結果を
ブラウザーの印刷画面へ送り、紙に印刷することもPDFとして保存することもできます。**

ImposiaはReactアプリで動く、ブラウザー専用の文書ツールです。次のページを
最後まで組み立ててから画面を切り替えるため、クライアント側の更新が続いても
未完成の文書は表示されません。プレビューと印刷は同じページを使います。
確定したソースの意味構造は、リフロー型EPUB 3.3 `Blob`としても出力できます。

`print()`を呼ぶと、ブラウザー標準の印刷画面が開きます。プリンターを選べば
紙へ印刷でき、**PDFとして保存**を選べばPDFファイルを作れます。Imposiaが
別のPDFレンダラーを動かしたり、PDFバイトを返したりするわけではありません。
...
| HTMLからPDFを作るために別のレンダラーが必要 | アプリのプレビューとPDFがずれる | `print()`で完成したページをブラウザーの印刷画面へ送り、PDFとして保存できる |
...
| :--- | ---: | ---: | ---: |
| 単語 1 つの編集後の再描画（50 ページ） | **30 ms** | 217 ms | 216 ms |
| 200 ページのページ分割 | **131 ms** | 846 ms | 2,140 ms |
| ブラウザー用バンドル全体、gzip | **58.5 KiB** | 94.2 KiB | 215.2 KiB |
...
ページ文書を表示し、同じ結果を印刷またはPDFとして保存します。
...
`print()`はViewerに表示中のページをブラウザー標準の印刷画面へ送ります。
紙に出す場合はプリンターを、ファイルに残す場合は**PDFとして保存**を選びます。
...
安定した対応範囲には、CSS の用紙サイズキーワード（A3〜A5、B4、B5、
Letter、Legal、Ledger）、独自の絶対寸法、縦・横向き、ホスト余白、
`:nth()` を含む `@page` セレクター、16 個すべてのマージンボックスが含まれます。
マージンボックスにはフォント・色・配置のスタイルを指定できます。
