...
허용된 에셋을 해석하고, 하나의 표준 iframe에서 페이지를 나눈 뒤, 그 문서를
미리보기와 네이티브 인쇄까지 그대로 유지합니다. 동일한 최신 커밋 시맨틱
...
| :--- | :--- | :--- |
| 미리보기와 인쇄가 달라짐 | 화면마다 레이아웃을 다시 실행함 | 하나의 표준 iframe이 페이지네이션, 표시, 네이티브 인쇄까지 유지됨 |
...
| **페이지네이션** | 페이지 지오메트리, 지원되는 `@page` 규칙, 프래그먼테이션, 참조, 퍼블리싱 콘텐츠가 하나의 iframe에서 해석됩니다. |
| **표시** | Viewer와 React 화면이 페이지를 복제하거나 레이아웃을 재실행하지 않고 표준 iframe을 유지합니다. |
| **퍼블리싱** | 네이티브 인쇄는 해당 iframe을 대상으로 하며, EPUB은 최신 커밋 시맨틱 소스를 제한된 아카이브로 투영합니다. |

소스 변경에 실패하면 원자적으로 롤백됩니다. 이전에 커밋된 세대는 새 세대가
성공하거나 컨트롤러가 파괴될 때까지 유지됩니다.
...
| [`@imposia/core`](./packages/core) | 표준 페이지 문서 런타임 | React 없이 생명주기, 페이지네이션, resolver, extension, 인쇄, EPUB을 직접 제어할 때 |
...
| 상태 | 포함되는 동작 |
| :--- | :--- |
| **Stable** | 브라우저 ESM API, 표준 iframe 생명주기, resolver 격리, 페이지 지오메트리, 지원되는 `@page` selector와 margin box, break, 네이티브 인쇄, 리플로우형 EPUB 내보내기 |
...
브라우저 E2E suite, 의존성 라이선스 감사를 실행합니다. 전체 gate와 캡처된
