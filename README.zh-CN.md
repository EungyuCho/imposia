...
<p align="center">
  ...
  <br/>
  <sub>在浏览器中原子化分页 HTML/CSR，并提供 React 预览、原生打印与语义化 EPUB 导出。</sub>
</p>
...
**将当前 HTML 与 CSS 转换为可检查的浏览器页面，并避免声明流内容在分页边界
丢失或重复。**
...
使用。即使 CSR 更新快速连续发生，也会在完整的新版本准备好之前保留上一次提交。
最新提交的语义源还可以导出为可重排 EPUB 3.3 `Blob`。
...
| :--- | :--- | :--- |
| 内容跨越分页边界 | 片段丢失、重复或顺序错乱 | 声明的验证样例在拼接页面后与源顺序完全一致；不支持的情况会警告，而不会假装成功 |
| 分页期间 CSR 状态变化 | 预览暴露不完整或过期版本 | 暂存期间保留上一次提交，仅用完整的最终版本替换 |
