...
Executed on 2026-07-18. This evidence covers the current Chromium-reference multi-page `@imposia/core` canonical paginator, its resolver-mediated asset boundary, the canonical-iframe Viewer surface, and the ownership move of the legacy PDF renderer to `@imposia/node`; it does not claim completion of the full target or Node PDF output from the Core paginator.
...
| Browser-Core matrix | Browser-Core matrix across Chromium, Firefox, and WebKit | 28 tests passed; 2 Firefox/WebKit print cases were expected skips. Chromium is the pagination reference. |
| Canonical browser pagination | Chromium fragmentation E2E coverage | Real A4 canonical pages are produced from ordered flow; all 84 markers occur exactly once, page metadata and recto/verso sides match the DOM, and the canonical iframe Viewer navigates the resulting page count. |
