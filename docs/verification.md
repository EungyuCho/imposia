...
| Exact source continuity across committed page boundaries and rapid CSR replacement | `CI=true pnpm exec playwright test tests/e2e/browser-core-conformance-performance.spec.ts --grep "records exact source continuity" --workers=1 --reporter=line`; `CI=true pnpm exec playwright test tests/e2e/demo-showcase.spec.ts --grep "page-boundary continuity" --project=chromium --workers=1 --reporter=line` | Exit `0`; the cross-browser fixture records every content page's first/last source token and flattens to all 96 tokens exactly once and in order. The public React lab retains one canonical iframe through three rapid source revisions and exposes the exact final ledger. | Playwright JSON attachments and repository terminal output |
