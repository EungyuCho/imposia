...
`CI=true pnpm check`: 354 browser scenarios passed, 120 intentional platform or
structural-reference scenarios were skipped, no scenario failed, and the E2E
matrix exited `0` (474 total scenarios, 2.9 minutes). The same gate also passed
8 site scenarios, 23 Vitest scenarios, the production audit, and the license
allowlist.
...
| Normalized page media, `@page` selectors, margin boxes, unsupported diagnostics, and print CSS | `CI=true ./node_modules/.bin/playwright test tests/e2e/browser-core-page-media.spec.ts tests/e2e/browser-core-print.spec.ts tests/e2e/browser-core-print-multipage.spec.ts tests/e2e/browser-core-lifecycle.spec.ts --project=chromium --workers=1 --reporter=line` | Exit `0`; native print clones the accepted canonical pages into a top-document isolated host, invokes the top window, preserves latest-generation selection, and prints every composed page even when caller `html`/`body` CSS constrains viewport height and overflow | [page-media-green.md](../.omo/evidence/browser-publishing-coverage/page-media-green.md), [browser-core-canonical-print.pdf](../.omo/evidence/browser-core-canonical-print.pdf) |
...
| Exact source continuity across committed page boundaries and rapid CSR replacement | `CI=true pnpm exec playwright test tests/e2e/browser-core-conformance-performance.spec.ts --grep "records exact source continuity" --workers=1 --reporter=line`; `CI=true pnpm exec playwright test tests/e2e/demo-playground.spec.ts --grep "half-built page set" --workers=1 --reporter=line` | Exit `0`; the cross-browser fixture records every content page's first/last source token and flattens to all 96 tokens exactly once and in order. The public Playground sends a burst of rapid edits and samples every rendered frame: no frame shows a page count other than one its generation committed, and a rejected update leaves the committed pages on screen. | Playwright JSON attachments and repository terminal output |
| Cooperative pagination equivalence, deep-node yielding, progress, and supersession | `CI=true ./node_modules/.bin/playwright test tests/e2e/browser-core-cooperative-pagination.spec.ts tests/e2e/browser-core-lifecycle.spec.ts --project=chromium --reporter=line`; `CI=true ./node_modules/.bin/vitest run tests/core/page-document-assets-ready.test.ts` | Exit `0`; forced-yield and uninterrupted modes produce equal page structure, a single large text node crosses repeated scheduler checkpoints, a pending scheduler wait aborts before the winning generation starts, one staging iframe is retained, provisional progress matches live page allocation, and mounted fonts/images settle before measurement. | Repository terminal output |
...
| Spread cover pairing, responsive fallback, page identity, and visual geometry | `CI=true ./node_modules/.bin/playwright test tests/e2e/spread-cover-mode.spec.ts` | Exit `0`; Chromium, Firefox, and WebKit preserve one canonical iframe, generation, and global page sequence across mode and keyboard navigation. Structural geometry assertions run on every engine and platform: cover-slot symmetry (the blank left slot mirrors one page plus the gutter), equal page widths and aligned sheet width for the 2–3 pair, and a fitted aspect-preserving single page at the 375 px fallback. | Repository terminal output |
