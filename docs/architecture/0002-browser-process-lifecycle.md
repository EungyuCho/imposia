...
Status: accepted for the legacy PDF path; Chromium lifecycle is now owned by `@imposia/node` in the current implementation.

`@imposia/node` launches the pinned Chromium process with Playwright's public `BrowserServer` API and connects through its loopback endpoint. Legacy PDF renders reuse that browser connection. One successfully reset local-only page may remain idle; concurrent overflow gets an independent page, while failed, timed-out, remote-enabled, and excess pages are closed. Reusable pages have routes removed and navigate to `about:blank` before entering the one-slot pool.
