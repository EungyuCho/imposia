...
- The PDF.js Viewer is canvas-first. Search, selectable text layers, annotations, forms, and editing are outside v1. The canonical-iframe Viewer retains ordinary page DOM and is currently Chromium-reference support.
...
The current Core/Viewer/CLI entrypoints are not aliases for the target API. `@imposia/node` currently contains the legacy PDF renderer and has not yet been changed to invoke the browser paginator; Core supplies Chromium-reference multi-page canonical pagination; and `mountPageViewer()` presents that iframe while `mountViewer()` remains the PDF.js surface. Migration can ship only behind a Chromium-reference gate that proves canonical structural equality and confirms sandbox, CSP, resolver, and resource-revocation behavior. If that gate fails, the stable `@imposia/node` PDF renderer and PDF.js Viewer remain the release path; the target is not silently substituted.
