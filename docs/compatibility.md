...
| Canonical page DOM, isolation, resolver boundary, abort, rollback, and cleanup | Stable | Core owns one sanitized iframe. Authored resources never fetch directly; discovered HTML/CSS assets pass through the host `assetResolver`, become Core-owned Blob URLs, and are revoked on replacement, failure, or destroy. |
