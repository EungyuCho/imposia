...
The print host isolates page content and generation styles, but CSS `@page` rules are document-level. Avoid a competing unnamed top-level `@page` rule in the host application while printing, because it can interact with Imposia's hoisted print-page rules.
