...
- **Print / Save as PDF** opens the browser's native print dialog for the page currently shown in the Viewer. Choose a printer or **Save as PDF** there. The method does not return PDF bytes.
...
If you update `source`, Imposia keeps the current page visible until the next version is ready. Read [How preview and output stay consistent](/en/docs/concepts/publishing-model) before adding asset loading or production export controls.

Use the [API reference](/en/docs/api) when you need every React handle method, Core controller option, Publication reader action, or Viewer lifecycle constraint.
