...
`useImposiaPublication`은 Publication 컴포넌트의 snapshot, revision, 옵션, 콜백을 받습니다. 스냅샷 참조나 `snapshotRevision`이 바뀔 때 갱신되고, `publicationOptionsRevision`은 컨트롤러를 다시 마운트합니다. 두 훅 모두 정리 단계에서 컨트롤러를 제거하고 `AbortError`를 수명 주기 취소로 봅니다.
