...
每个 `limits` 字段都可省略，省略时使用默认值。数值可以随意调低。只有带最大值的四个字段可以调高到该最大值，其余字段不能高于默认值。

| 字段 | 默认值 | 最大值 |
| --- | --- | --- |
| `maxInputBytes` | 5 MiB（`5 * 1024 * 1024`） | 32 MiB（`32 * 1024 * 1024`） |
| `maxNodes` | 100,000 | 1,000,000 |
| `maxAssetBytes` | 25 MiB（`25 * 1024 * 1024`） | 同默认值 |
| `maxAssetDepth` | 8 | 同默认值 |
| `maxAssetReferences` | 512 | 同默认值 |
| `resourceDeadlineMs` | 30,000 | 300,000 |
| `maxPages` | 10,000 | 50,000 |
| `maxLayoutPasses` | 8 | 同默认值 |
| `maxGeneratedFragments` | 400,000 | 同默认值 |
| `maxGeneratedRecords` | 10,000 | 同默认值 |
...
| `UNSUPPORTED_LAYOUT` | 受限子集之外的布局保持原子，并因此造成损失：超出页面、从自身盒子侧向溢出，或内部的强制分页被忽略。能完整放进页面时不报告。 |
