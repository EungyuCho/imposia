...
| `goToPage(page)`, `nextPage()`, `previousPage()` | `void` | Move through the committed global page sequence; throw before ready/after unmount. |
| `setZoom(zoom)` | `void` | Rounds and clamps the presentation zoom; throws before ready/after unmount. |
...
| `exportEpub(options)` | `Promise<Blob>` | Exports the current commit as a reflowable EPUB; rejects before ready/after unmount. |
