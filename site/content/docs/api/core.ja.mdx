...
`limits` の各フィールドは任意で、省略すると既定値が適用されます。値はいくらでも下げられます。上限値のある 4 つのフィールドだけはその上限値まで上げられ、それ以外は既定値より上げられません。

| フィールド | 既定値 | 上限値 |
| --- | --- | --- |
| `maxInputBytes` | 5 MiB（`5 * 1024 * 1024`） | 32 MiB（`32 * 1024 * 1024`） |
| `maxNodes` | 100,000 | 1,000,000 |
| `maxAssetBytes` | 25 MiB（`25 * 1024 * 1024`） | 既定値と同じ |
| `maxAssetDepth` | 8 | 既定値と同じ |
| `maxAssetReferences` | 512 | 既定値と同じ |
| `resourceDeadlineMs` | 30,000 | 300,000 |
| `maxPages` | 10,000 | 50,000 |
| `maxLayoutPasses` | 8 | 既定値と同じ |
| `maxGeneratedFragments` | 400,000 | 既定値と同じ |
| `maxGeneratedRecords` | 10,000 | 既定値と同じ |
...
| `UNSUPPORTED_LAYOUT` | 制約付きサブセットの外のレイアウトをそのまま残し、その結果として損失が出た。ページからあふれた、ボックスの横にはみ出した、内部の強制改ページが無視された、のいずれか。ページに収まる場合は報告しない。 |
