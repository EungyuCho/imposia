...
| Paginate a 99-page article | 125 ms |
| Change one word in that article and recommit | 121 ms |
| First rendered frame after a commit | 1.9 ms |
| Paginate a 100-entry Publication where every entry has its own `<style>` | 94 ms |

Measured on September 23, 2026 with Chromium 149 on an Apple M4. Times are medians of seven runs, and the sizes exclude React and React DOM. To measure on your own machine, run `pnpm build` and then `pnpm benchmark` in the repository.
